...
func Update(w http.ResponseWriter, r *http.Request) {
	if r.Method == "POST" {
		...
		precoConvertidoParaFloat, err := strconv.ParseFloat(preco, 64)
		if err != nil {
			log.Println("Erro na conversão de PREÇO para FLOAT64: ", err)
		}
		quantidadeConvertidaParaInt, err := strconv.Atoi(quantidade)
		if err != nil {
			log.Println("Erro na conversão de QUANTIDADE para INT: ", err)
		}
		models.AtualizaProduto(idConvertidaParaInt, nome, descricao, quantidadeConvertidaParaInt, precoConvertidoParaFloat)
	}
	http.Redirect(w, r, "/", 301)
}
